...
export async function previewProductUpload(form:FormData) {
  ...
  try { return {plan:await planProductImport(prisma,upload.csv,selectedSource(form)),sheets:'sheets' in upload ? upload.sheets : [],selectedSheet:'selectedSheet' in upload ? upload.selectedSheet : undefined}; }
...
export async function confirmProductUpload(form:FormData,digest:string) {
  ...
  try {
    const counts=await applyProductImport(prisma,upload.csv,digest,selectedSource(form));
