...
export function RosaReviewCard({group,plan,manual,disabled,onResolve,onCreateAccount}:{group:Group;plan:RosaPlan;manual:RosaManualGroupChoice|undefined;disabled:boolean;onResolve:(groupKey:string,kind:ChoiceKind,field:string,value:number|null)=>void;onCreateAccount?:(groupKey:string,field:'Customer'|'VAR'|'End User',source:string)=>void}){
  const roleItems=[
