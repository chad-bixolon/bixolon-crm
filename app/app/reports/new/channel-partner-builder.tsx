...
export async function ChannelPartnerBuilder({params,actor,saved}:{params:Params;actor:Actor;saved:Saved}){
  ...
  return <Content><PageHeader eyebrow="Reports" title={saved?'Edit Channel / Partner Report':'Channel / Partner Report'} description="See Opportunity pipeline associated with participating partners." action={<Link className="btn-secondary" href="/reports">Reports</Link>}/>
    <div className="mb-2 flex flex-wrap gap-2"><Link className="btn-secondary" href="/reports/new?reportType=PIPELINE">Pipeline</Link><Link className="btn-secondary" href="/reports/new?reportType=ACCOUNT_ACTIVITY">Account Activity</Link><Link className="btn-secondary" href="/reports/new?reportType=PRODUCT_PERFORMANCE">Product Performance</Link><Link className="btn-secondary" href="/reports/new?reportType=CHANNEL_PARTNER">Channel / Partner</Link></div>
