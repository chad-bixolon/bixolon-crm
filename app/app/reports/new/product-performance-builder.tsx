...
export async function ProductPerformanceBuilder({params,actor,saved}:{params:Params;actor:Actor;saved:Saved}){
  ...
  const definition=reportRegistry.PRODUCT_PERFORMANCE;
  ...
  return <Content><PageHeader eyebrow="Reports" title={saved?'Edit Product Performance Report':'Product Performance Report'} description="Analyze OpportunityProduct lines using their saved commercial prices." action={<Link className="btn-secondary" href="/reports">Reports</Link>}/>
    <div className="mb-2 flex flex-wrap gap-2"><Link className="btn-secondary" href="/reports/new?reportType=PIPELINE">Pipeline</Link><Link className="btn-secondary" href="/reports/new?reportType=ACCOUNT_ACTIVITY">Account Activity</Link><Link className="btn-secondary" href="/reports/new?reportType=PRODUCT_PERFORMANCE">Product Performance</Link></div>
    <form method="get" className="panel report-builder p-3 md:p-4"><input type="hidden" name="configured" value="1"/><input type="hidden" name="reportType" value="PRODUCT_PERFORMANCE"/>{saved&&<input type="hidden" name="reportId" value={saved.id}/>}
    ...
    <form action={saveReportAction} className="panel mt-4 grid gap-3 p-4 sm:grid-cols-2 md:p-5"><h2 className="text-lg font-semibold sm:col-span-2">Save This Report</h2><input type="hidden" name="reportType" value="PRODUCT_PERFORMANCE"/><input type="hidden" name="configuration" value={JSON.stringify(config)}/>{saved&&<input type="hidden" name="reportId" value={saved.id}/>}
