...
export default async function ContactsPage({ searchParams }: { searchParams: Promise<ContactFilters> }) {
  ...
  return <Content>
    <PageHeader eyebrow="CRM records" title="Contacts" description="People with or without an account." action={<Link className="btn-primary" href="/contacts/new">New contact</Link>}/>
    <form method="get" className="panel mb-5 grid gap-3 bg-orange-50/50 p-4 sm:grid-cols-2 lg:grid-cols-5" aria-label="Filter contacts">
      <div><label className="label" htmlFor="q">Search</label><input className="field" id="q" name="q" defaultValue={filters.q ?? ""} placeholder="Name or email"/></div>
      <div><label className="label" htmlFor="active">Status</label><select className="field" id="active" name="active" defaultValue={filters.active ?? ""}><option value="">All</option><option value="active">Active</option><option value="inactive">Inactive</option><option value="archived">Archived</option></select></div>
      <div><label className="label" htmlFor="accountId">Account</label><select className="field" id="accountId" name="accountId" defaultValue={filters.accountId ?? ""}><option value="">All accounts</option><option value="unassigned">Unassigned</option>{accounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></div>
      <div><label className="label" htmlFor="marketingPreference">Marketing Preference</label><select className="field" id="marketingPreference" name="marketingPreference" defaultValue={filters.marketingPreference ?? ""}><option value="">All preferences</option><option value="UNKNOWN">Unknown / Not Confirmed</option><option value="OPTED_IN">Opted In</option><option value="OPTED_OUT">Opted Out</option></select></div>
      <div className="flex items-end gap-2"><button className="btn-primary">Apply</button><Link className="btn-secondary" href="/contacts">Clear</Link></div>
    </form>
