...
export default async function ContactsPage({ searchParams }: { searchParams: Promise<ContactFilters> }) {
  ...
  const [{ contacts, count, page, pages }, accounts] = await Promise.all([listContacts(prisma, filters), prisma.account.findMany({ where: { status: 'ACTIVE', archivedAt: null }, orderBy: { name: "asc" }, select: { id: true, name: true } })]);
  ...
  return <Content>
    ...
    <form method="get" className="panel filter-panel filter-grid mb-5" aria-label="Filter contacts">
      ...
      <div><label className="label" htmlFor="active">Status</label><select className="field filter-control" id="active" name="active" defaultValue={filters.active ?? ""}><option value="">Current</option><option value="all">All</option><option value="active">Active</option><option value="inactive">Inactive</option><option value="archived">Archived</option></select></div>
