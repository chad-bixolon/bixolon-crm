...
export default async function ContactPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <Content><PageHeader eyebrow="Contacts" title={`${c.firstName} ${c.lastName}`} description={`Contact #${id}`} action={<div className="flex gap-2"><Link className="btn-secondary" href="/contacts">All contacts</Link>{!c.archivedAt && <Link className="btn-primary" href={`/contacts/${id}/edit`}>Edit contact</Link>}</div>}/>
    ...
    <div className="panel p-6"><dl className="grid gap-5 sm:grid-cols-2">{[["Account", c.account ? <Link key="account" className="text-orange-800 underline" href={`/accounts/${c.accountId}`}>{c.account.name}</Link> : "Unassigned"], ["Title", c.title ?? "—"], ["Email", c.email ? <a key="email" className="text-orange-800 underline" href={`mailto:${c.email}`}>{c.email}</a> : "—"], ["Office phone", c.phone ?? "—"], ["Mobile phone", c.mobile ?? "—"]].map(([label, value]) => <div key={String(label)}><dt className="label">{label}</dt><dd className="text-sm">{value}</dd></div>)}</dl></div>
