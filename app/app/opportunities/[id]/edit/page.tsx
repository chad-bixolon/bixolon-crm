...
export default async function EditOpportunityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [opportunity, options, labels] = await Promise.all([prisma.opportunity.findUnique({ where: { id }, include: { participants: { include: { roles: true } }, products: { where: { archivedAt: null } } } }), opportunityOptions(prisma), getLabels(prisma)]);
