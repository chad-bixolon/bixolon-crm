...
export default async function TradeShowPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{q?:string;rep?:string;status?:string;routing?:string;account?:string;contact?:string;followUp?:string;bulk?:string}> }) {
  ...
  if (!show) notFound();
  const [kpiRows,report] = await Promise.all([prisma.tradeShowLead.findMany({ where: { AND: [{ tradeShowId: id }, tradeShowLeadReadWhere(actor)] }, select: { assignedSalesRepUserId: true, status: true,routing:true } }),executeTradeShowReport(prisma,actor,{...defaultReportConfiguration('TRADE_SHOW'),filters:[{field:'tradeShowId',operator:'eq',value:id}]})]);
