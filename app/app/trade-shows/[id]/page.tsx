...
export default async function TradeShowPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{q?:string;rep?:string;status?:string;routing?:string;account?:string;contact?:string;followUp?:string;bulk?:string}> }) {
      ...
      <div className="p-5">
        ...
        <form className="mt-4 rounded-md border border-orange-200 bg-orange-50/60 p-3" method="get" aria-label="Filter Trade Show leads">
          <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
            ...
            <div><label className="mb-1 block text-xs font-semibold text-slate-700" htmlFor="lead-routing">Routing</label><select className="field h-9 min-w-0 px-2.5 py-1.5 text-sm" id="lead-routing" name="routing" defaultValue={filters.routing??''}><option value="">All routing</option>{routings.map(routing=><option key={routing} value={routing}>{routingLabels[routing as TradeShowLeadRouting]}</option>)}</select></div>
            ...
            <div><label className="mb-1 block text-xs font-semibold text-slate-700" htmlFor="lead-contact">Contact Resolution</label><select className="field h-9 min-w-0 px-2.5 py-1.5 text-sm" id="lead-contact" name="contact" defaultValue={filters.contact??''}><option value="">All</option><option value="linked">Linked</option><option value="unresolved">Needs Resolution</option></select></div>
