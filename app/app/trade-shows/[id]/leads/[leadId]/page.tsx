...
export default async function TradeShowLeadPage({ params }: { params: Promise<{ id: string; leadId: string }> }) {
  ...
  return <Content><PageHeader eyebrow={lead.tradeShow.name} title={`${lead.firstName} ${lead.lastName}`} description={`Trade Show Lead #${lead.id}`} action={<div className="flex gap-2"><Link className="btn-secondary" href={`/trade-shows/${tradeShowId}`}>Back to Trade Show</Link>{!lead.tradeShow.archivedAt && canEditTradeShowLead(actor, lead) && <Link className="btn-primary" href={`/trade-shows/${tradeShowId}/leads/${lead.id}/edit`}>Edit Lead</Link>}</div>}/>
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <section className="panel p-5"><h2 className="mb-4 text-lg font-semibold">Source Details / Provenance</h2><dl className="grid gap-4 sm:grid-cols-2">{row('Captured', when(lead.capturedAt))}{row('Imported', when(lead.importedAt))}{row('File', lead.sourceFileName)}{row('Sheet / Row', `${lead.sourceSheet} / ${lead.sourceRow}`)}{row('Source Notes', lead.sourceNotes)}</dl><details className="mt-4"><summary className="cursor-pointer text-sm font-medium text-orange-800">Original source fields</summary><pre className="mt-3 overflow-x-auto whitespace-pre-wrap break-words rounded bg-slate-50 p-3 text-xs">{JSON.stringify(lead.rawSourceData, null, 2)}</pre></details></section>
