...
async function upload(form:FormData){const file=form.get('file');if(!(file instanceof File))throw new Error('Choose a .xls file.');if(!file.name.toLowerCase().endsWith('.xls')||!file.size||file.size>MAX_TRADE_SHOW_FILE_BYTES)throw new Error('Choose a nonempty .xls file no larger than 2 MB.');return {buffer:Buffer.from(await file.arrayBuffer()),filename:file.name};}
