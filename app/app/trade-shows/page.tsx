...
export default async function TradeShowsPage({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  return <Content><PageHeader eyebrow="Marketing" title="Trade Shows" description="Events and their captured lead outcomes." action={can(actor, 'trade-shows.manage') ? <Link className="btn-primary" href="/trade-shows/new">Create Trade Show</Link> : undefined}/>
    ...
    <div className="panel min-w-0 max-w-full"><TableScroll label="Trade Shows"><table className="w-full min-w-[900px] text-left text-sm"><thead className="border-b bg-slate-50 text-xs uppercase text-slate-500"><tr>{['Trade Show','Dates','Location','Leads','Assigned','Contacted','Qualified','Converted'].map(label => <th className="px-4 py-3" key={label}>{label}</th>)}</tr></thead><tbody className="divide-y">{shows.map(show => { const kpi = tradeShowKpis(show.leads); return <tr className="even:bg-slate-50/60 hover:bg-orange-50/50 focus-within:bg-orange-50/50" key={show.id}><td className="max-w-64 break-words px-4 py-3"><Link className="font-semibold text-orange-800 hover:underline" href={`/trade-shows/${show.id}`}>{show.name}</Link></td><td className="px-4 py-3">{date(show.startDate)}{show.endDate && show.endDate.getTime() !== show.startDate?.getTime() ? ` – ${date(show.endDate)}` : ''}</td><td className="max-w-64 break-words px-4 py-3">{show.location ?? '—'}</td><td className="px-4 py-3">{kpi.total}</td><td className="px-4 py-3">{kpi.assigned}</td><td className="px-4 py-3">{kpi.contacted}</td><td className="px-4 py-3">{kpi.qualified}</td><td className="px-4 py-3">{kpi.converted}</td></tr>; })}</tbody></table></TableScroll>{!shows.length && <p className="p-8 text-center text-sm text-slate-500">No {archived ? 'archived' : 'active'} Trade Shows yet.</p>}</div>
  </Content>;
}
