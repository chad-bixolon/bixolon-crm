...
export async function GET(request:NextRequest,{params}:{params:Promise<{id:string}>}){
  ...
  const rows=await prisma.contact.findMany({where:{id:{in:resolved.selected},...(ready?{marketingPreference:"OPTED_IN"}:{})},include:audienceContactInclude,orderBy:[{lastName:"asc"},{firstName:"asc"},{id:"asc"}]});
