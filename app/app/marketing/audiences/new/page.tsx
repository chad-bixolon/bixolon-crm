...
export default async function NewAudiencePage(){const actor=await requirePermission("marketing.read");if(!can(actor,"marketing.write"))return <Content><PageHeader title="New Marketing Audience"/><div className="panel p-6">You have read-only Marketing access.</div></Content>;const options=await audienceBuilderOptions(prisma);return <Content><PageHeader eyebrow="Marketing Audiences" title="New Marketing Audience" description="Start with opted-in Contacts, then refine the live CRM filters."/><MarketingAudienceForm options={options}/></Content>}
