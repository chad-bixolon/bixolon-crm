...
export async function readSnapshot(db) {
  const data={};
  for (const [key, model] of Object.entries(models)) data[key]=await db[model].findMany({select:selects[key]});
  return data;
}
...
export async function writeReport(result, outputDir, target) {
  await fs.mkdir(outputDir,{recursive:true,mode:0o700});
  await fs.chmod(outputDir,0o700);
  const files=[];
  const emit=async(name,rows,columns)=>{await writePrivate(path.join(outputDir,name),csv(rows,columns));files.push(name);};
...
async function main() {
  ...
  if(args.some(arg=>arg.startsWith('--')&&!['--expect-host','--output'].includes(arg))) throw new Error('Unsupported argument. No query run.');
  ...
  try {
    ...
    const files=await writeReport(result,output,target);
...
if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)) main().catch(error=>{console.error(`Audit failed: ${error.code??error.name??'Error'}. Details withheld to protect connection credentials.`);process.exitCode=1;});
