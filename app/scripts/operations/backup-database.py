...
SAFE_LABEL = re.compile(r'[A-Za-z0-9][A-Za-z0-9_-]{0,63}\Z')


def safe_label(value):
    if not SAFE_LABEL.fullmatch(value):
        raise argparse.ArgumentTypeError(
            'label must be 1-64 characters using only ASCII letters, digits, underscores, or hyphens'
        )
    return value
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        '--label', type=safe_label, default='migration', metavar='SAFE_LABEL',
        help='backup label (default: migration; allowed: ASCII letters, digits, _ and -)',
    )
...
def main(argv=None):
    ...
    path = folder / f'bixolon_crm_before_{options.label}_{timestamp}.dump'
