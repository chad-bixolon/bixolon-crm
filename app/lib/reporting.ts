...
export const reportTypes = ['PIPELINE','ACCOUNT_ACTIVITY','PRODUCT_PERFORMANCE','CHANNEL_PARTNER','PROJECT_INITIATIVE','PRICE_EXCEPTION_USAGE','TRADE_SHOW'] as const;
...
const priceExceptionUsageDefinition: ReportTypeDefinition = {
  label:'Price Exception Usage Report',description:'Review Opportunities using Price Exception pricing by account, product, sales rep, and PE.',grain:'OpportunityProduct',implemented:true,
  semanticNote:'Price Exception Usage shows where PE pricing is being used on Opportunity products. Line Value uses the actual Opportunity Unit Price, while PE Approved Price is shown separately for reference. Totals remain separated by currency and unit.',
  filters:{ownerId:{label:'Sales Rep',operators:['eq']},accountId:{label:'Account',operators:['eq']},opportunityId:{label:'Opportunity',operators:['eq']},stageId:{label:'Opportunity Stage',operators:['eq']},competitorId:{label:'Competitor',operators:['eq']},forecastCategory:{label:'Forecast Category',operators:['eq']},status:{label:'Status',operators:['eq']},closeDate:{label:'Expected Close Date',operators:['preset','between']},productId:{label:'Product',operators:['eq']},skuId:{label:'SKU',operators:['eq']},productCategoryId:{label:'Product Category',operators:['eq']},priceExceptionId:{label:'Price Exception',operators:['eq']},peSalespersonId:{label:'Price Exception Sales Rep',operators:['eq']},moqStatus:{label:'MOQ Status',operators:['eq']},overrideStatus:{label:'Override Status',operators:['eq']},currency:{label:'Currency',operators:['eq']}},
  ...
  groupings:{owner:'Sales Rep',peSalesperson:'Price Exception Sales Rep',account:'Account',priceException:'Price Exception',product:'Product',productCategory:'Product Category',opportunity:'Opportunity',stage:'Opportunity Stage',forecastCategory:'Forecast Category',currency:'Currency'},
  metrics:{lineValue:'Line Value',quantity:'Quantity',opportunityCount:'Opportunity Count',productLineCount:'Product Line Count',priceExceptionCount:'Distinct Price Exceptions Used',averageApprovedUnitPrice:'Average PE Approved Price',averageActualUnitPrice:'Average Opportunity Unit Price'},
  sorts:{lineValue:'Line Value',quantity:'Quantity',opportunity:'Opportunity',product:'Product',peCode:'Price Exception',closeDate:'Expected Close Date',owner:'Sales Rep'},
};

const tradeShowDefinition: ReportTypeDefinition = {
  label:'Trade Show Performance',description:'Review Trade Show leads, follow-up, conversions, and the sales pipeline generated from each event.',grain:'Trade Show Lead',implemented:true,
  semanticNote:'Trade Show attribution uses only the originating converted lead. Opportunity values are counted once, use active product lines, and remain separated by currency.',
  filters:{tradeShowId:{label:'Trade Show',operators:['eq']},showDate:{label:'Trade Show date',operators:['preset','between']},ownerId:{label:'Assigned Sales Rep',operators:['eq']},leadStatus:{label:'Lead Status',operators:['eq']},routing:{label:'Routing',operators:['eq']},referralPartnerId:{label:'Referral Partner',operators:['eq']},converted:{label:'Converted',operators:['eq']},accountLinked:{label:'Account Linked',operators:['eq']},contactLinked:{label:'Contact Linked',operators:['eq']},followUpStatus:{label:'Follow-Up Status',operators:['eq']},competitorId:{label:'Resolved Competitor',operators:['eq']},productInterest:{label:'Product Interest',operators:['contains']},search:{label:'Lead, company, or email',operators:['contains']},stageId:{label:'Opportunity Stage',operators:['eq']},forecastCategory:{label:'Forecast Category',operators:['eq']},currency:{label:'Currency',operators:['eq']}},
  columns:{tradeShow:'Trade Show',lead:'Lead',company:'Company',assignedRep:'Assigned Rep',leadStatus:'Lead Status',routing:'Routing',referralPartner:'Referral Partner',account:'Account',contact:'Contact',capturedDate:'Captured Date',followUp:'Follow-Up',competitor:'Competitor',competitorSourceText:'Competitor Source Text',productInterest:'Product Interest',opportunity:'Converted Opportunity',opportunityOwner:'Opportunity Owner',opportunityStage:'Opportunity Stage',opportunityValue:'Opportunity Value',weightedValue:'Weighted Value',conversionDate:'Conversion Date',currency:'Currency'},
  groupings:{tradeShow:'Trade Show',assignedRep:'Assigned Sales Rep',leadStatus:'Lead Status',routing:'Routing',referralPartner:'Referral Partner',competitor:'Resolved Competitor',opportunityStage:'Opportunity Stage',forecastCategory:'Forecast Category',currency:'Currency'},
  metrics:{totalLeads:'Total Leads',assignedLeads:'Assigned Leads',unreviewedLeads:'Unreviewed Leads',bixolonSalesLeads:'BIXOLON Sales Leads',partnerReferredLeads:'Partner-Referred Leads',marketingFollowUpLeads:'Marketing Follow-Up Leads',referralRate:'Referral Rate',newLeads:'New',contactedLeads:'Contacted',qualifiedLeads:'Qualified',convertedLeads:'Converted',disqualifiedLeads:'Disqualified',conversionRate:'Conversion Rate',opportunityCount:'Opportunities Created',pipeline:'Open Pipeline',weightedPipeline:'Weighted Pipeline',commit:'Commit',closedWonValue:'Closed Won Value',closedWonOpportunityCount:'Closed Won Opportunity Count'},
  sorts:{tradeShow:'Trade Show',lead:'Lead',company:'Company',assignedRep:'Assigned Rep',leadStatus:'Lead Status',capturedDate:'Captured Date',followUp:'Follow-Up Date',opportunityStage:'Opportunity Stage',opportunityValue:'Opportunity Value',conversionDate:'Conversion Date'},
};
...
function validFilterValue(filter: ReportFilter) {
  if (['ownerId','stageId','competitorId','accountId','accountOwnerId','productCategoryId','productId','skuId','odmCustomerAccountId','projectId','minDays','opportunityId','priceExceptionId','peSalespersonId','tradeShowId','referralPartnerId'].includes(filter.field)) return positiveInteger(filter.value);
  ...
  if (['converted','accountLinked','contactLinked'].includes(filter.field)) return typeof filter.value === 'boolean';
  if (filter.field === 'leadStatus') return ['NEW','CONTACTED','QUALIFIED','CONVERTED','DISQUALIFIED'].includes(String(filter.value));
  if (filter.field === 'routing') return ['UNREVIEWED','BIXOLON_SALES','REFERRED_TO_PARTNER','MARKETING_FOLLOW_UP'].includes(String(filter.value));
  if (filter.field === 'followUpStatus') return ['OVERDUE','SCHEDULED','MISSING','NEW_NOT_CONTACTED','QUALIFIED_NOT_CONVERTED'].includes(String(filter.value));
  if (['productInterest','search'].includes(filter.field)) return typeof filter.value === 'string' && filter.value.trim().length > 0 && filter.value.length <= 200;
  ...
  if (filter.field === 'priceSource') return ['MANUAL','CATALOG','PRICE_EXCEPTION','ODM_CUSTOMER'].includes(String(filter.value));
  ...
  if (['closeDate','createdDate','startDate','targetEndDate','showDate'].includes(filter.field) && filter.operator === 'preset') return presets.includes(filter.value as typeof presets[number]);
  if (['closeDate','createdDate','startDate','targetEndDate','showDate'].includes(filter.field) && filter.operator === 'between') return object(filter.value) && exactKeys(filter.value, ['from','to']) && dateString(filter.value.from) && dateString(filter.value.to) && String(filter.value.from) <= String(filter.value.to);
  return false;
}

export function defaultReportConfiguration(reportType: CuratedReportType): ReportConfiguration {
  if (reportType === 'TRADE_SHOW') return {filters:[],groupBy:'tradeShow',sort:[{field:'capturedDate',direction:'desc'}],columns:['tradeShow','lead','company','routing','referralPartner','assignedRep','leadStatus','account','contact','followUp','opportunity','opportunityStage','opportunityValue','weightedValue','currency'],metrics:['totalLeads','unreviewedLeads','bixolonSalesLeads','partnerReferredLeads','marketingFollowUpLeads','referralRate','contactedLeads','qualifiedLeads','convertedLeads','conversionRate','opportunityCount','pipeline','weightedPipeline','commit','closedWonValue']};
...
export function validateReportConfiguration(reportType: unknown, input: unknown): ReportConfiguration {
  ...
  if (input.filters.length > 30 || input.sort.length > 3 || input.columns.length > 20 || input.metrics.length > 20) throw new Error('Report configuration is too large.');
...
export function canShareReport(actor: Actor) { return canCreateReport(actor) && ['ADMIN','SALES_MANAGER','MARKETING_MANAGER'].includes(actor.role); }
...
const productLineInclude = {product:{select:{name:true,sku:true,categoryId:true,category:{select:{name:true}}}},sku:{select:{partNumber:true,priceUnit:true,catalogSource:true,odmSubtype:true,odmCustomers:{select:{accountId:true,account:{select:{name:true}}}}}},opportunity:{select:{name:true,ownerId:true,owner:{select:{firstName:true,lastName:true}},stageId:true,stage:{select:{name:true}},forecastCategory:true,expectedCloseDate:true,currencyCode:true,participants:{select:{accountId:true,account:{select:{name:true}}}},projects:{select:{projectId:true,project:{select:{name:true}}}}}}} as const;
...
function productGroupValues(row:ProductLineDbRow, groupBy:string|null):{key:string;label:string}[] {
  ...
  if(groupBy==='odmCustomer') {
    if(row.sku?.catalogSource!=='ODM')return [{key:'not-odm',label:'Not ODM'}];
    if(row.priceSource==='ODM_CUSTOMER' && row.odmCustomerAccountId){const match=row.sku.odmCustomers.find(link=>link.accountId===row.odmCustomerAccountId);return [{key:String(row.odmCustomerAccountId),label:match?.account.name??`Account #${row.odmCustomerAccountId}`}];}
    if(row.sku.odmCustomers.length===1)return [{key:String(row.sku.odmCustomers[0].accountId),label:row.sku.odmCustomers[0].account.name}];
    if(row.sku.odmCustomers.length>1)return [{key:'multiple',label:'Multiple ODM Customers (manual context)'}];
    return [{key:row.sku.odmSubtype==='CUSTOMER_SPECIFIC'?'none':row.sku.odmSubtype?'not-customer-specific':'subtype-unset',label:row.sku.odmSubtype==='CUSTOMER_SPECIFIC'?'Unresolved ODM Customer':row.sku.odmSubtype?'No ODM Customer Required':'ODM Subtype Not Set'}];
  }
...
export async function executeProductPerformanceReport(client:PrismaClient, actor:Actor, rawConfig:unknown, now=new Date()):Promise<ProductPerformanceResult> {
  ...
  for(const filter of config.filters){
    ...
    else if(filter.field==='odmCustomerAccountId')lineClauses.push({sku:{odmCustomers:{some:{accountId:filter.value as number}}}});
  }
...
const tradeShowReportInclude = {
  tradeShow:true,assignedSalesRep:true,routedPartnerAccount:true,account:true,contact:true,competitor:true,
  convertedOpportunity:{include:{owner:true,stage:true,products:{where:{archivedAt:null}}}},
} as const;
type TradeShowReportDbRow = Prisma.TradeShowLeadGetPayload<{include:typeof tradeShowReportInclude}>;
export type TradeShowLeadMetrics = {totalLeads:number;assignedLeads:number;unreviewedLeads:number;bixolonSalesLeads:number;partnerReferredLeads:number;marketingFollowUpLeads:number;referralRate:number|null;newLeads:number;contactedLeads:number;qualifiedLeads:number;convertedLeads:number;disqualifiedLeads:number;conversionRate:number|null};
export type TradeShowCurrencyMetrics = {currency:string;opportunityCount:number;pipeline:string;weightedPipeline:string;commit:string;closedWonValue:string;closedWonOpportunityCount:number};
export type TradeShowMetrics = {leads:TradeShowLeadMetrics;amounts:TradeShowCurrencyMetrics[]};
export type TradeShowDetailRow = {id:number;tradeShowId:number;tradeShow:string;lead:string;company:string;assignedRep:string;leadStatus:string;routing:string;referralPartner:{id:number;name:string}|null;account:{id:number;name:string}|null;contact:{id:number;name:string}|null;capturedDate:string|null;followUp:string|null;competitor:string;competitorSourceText:string;productInterest:string;opportunity:{id:number;name:string}|null;opportunityOwner:string;opportunityStage:string;forecastCategory:string;opportunityValue:string|null;weightedValue:string|null;conversionDate:string|null;currency:string|null;canOpenOpportunity:boolean;groupKeys:string[]};
export type TradeShowReportResult = {reportType:'TRADE_SHOW';summary:TradeShowMetrics;groups:{key:string;label:string;metrics:TradeShowMetrics;leadIds:number[]}[];rows:TradeShowDetailRow[];currencies:string[];filterCount:number;semanticNote:string};

function tradeShowLeadMetrics(rows:TradeShowReportDbRow[]):TradeShowLeadMetrics {
  const converted=rows.filter(row=>row.status==='CONVERTED').length;
  const route=(row:TradeShowReportDbRow)=>row.routing??'UNREVIEWED',referred=rows.filter(row=>route(row)==='REFERRED_TO_PARTNER').length;
  return {totalLeads:rows.length,assignedLeads:rows.filter(row=>row.assignedSalesRepUserId!==null).length,unreviewedLeads:rows.filter(row=>route(row)==='UNREVIEWED').length,bixolonSalesLeads:rows.filter(row=>route(row)==='BIXOLON_SALES').length,partnerReferredLeads:referred,marketingFollowUpLeads:rows.filter(row=>route(row)==='MARKETING_FOLLOW_UP').length,referralRate:rows.length?Math.round(referred/rows.length*1000)/10:null,newLeads:rows.filter(row=>row.status==='NEW').length,contactedLeads:rows.filter(row=>['CONTACTED','QUALIFIED','CONVERTED'].includes(row.status)).length,qualifiedLeads:rows.filter(row=>['QUALIFIED','CONVERTED'].includes(row.status)).length,convertedLeads:converted,disqualifiedLeads:rows.filter(row=>row.status==='DISQUALIFIED').length,conversionRate:rows.length?Math.round(converted/rows.length*1000)/10:null};
}
function distinctTradeShowOpportunities(rows:TradeShowReportDbRow[]) {
  return [...new Map(rows.flatMap(row=>row.convertedOpportunity&&row.convertedOpportunity.archivedAt===null?[[row.convertedOpportunity.id,row.convertedOpportunity] as const]:[])).values()];
}
function tradeShowCurrencyMetrics(rows:TradeShowReportDbRow[]):TradeShowCurrencyMetrics[] {
  const opportunities=distinctTradeShowOpportunities(rows);
  return [...new Set(opportunities.map(item=>item.currencyCode))].sort().map(currency=>{
    const selected=opportunities.filter(item=>item.currencyCode===currency);
    const amount=(kind:'pipeline'|'weightedPipeline'|'commit'|'closedWonValue')=>selected.reduce((sum,item)=>{
      const value=opportunityTotal(item.products);
      const open=!item.stage.isClosed&&['PIPELINE','BEST_CASE','COMMIT'].includes(item.forecastCategory);
      if(kind==='pipeline')return sum.add(open?value:0);
      if(kind==='weightedPipeline')return sum.add(open?weightedValue(value,item.probability??item.stage.probability):0);
      if(kind==='commit')return sum.add(open&&item.forecastCategory==='COMMIT'?value:0);
      return sum.add(item.stage.isClosed&&item.stage.isWon?value:0);
    },new Prisma.Decimal(0)).toFixed(2);
    return {currency,opportunityCount:selected.length,pipeline:amount('pipeline'),weightedPipeline:amount('weightedPipeline'),commit:amount('commit'),closedWonValue:amount('closedWonValue'),closedWonOpportunityCount:selected.filter(item=>item.stage.isClosed&&item.stage.isWon).length};
  });
}
function tradeShowMetrics(rows:TradeShowReportDbRow[]):TradeShowMetrics {return {leads:tradeShowLeadMetrics(rows),amounts:tradeShowCurrencyMetrics(rows)};}
function tradeShowGroup(row:TradeShowReportDbRow,groupBy:string|null){
  const one=(key:string,label:string)=>({key,label});
  if(groupBy==='tradeShow')return one(String(row.tradeShowId),row.tradeShow.name);
  if(groupBy==='assignedRep')return one(String(row.assignedSalesRepUserId??'none'),row.assignedSalesRep?`${row.assignedSalesRep.firstName} ${row.assignedSalesRep.lastName}`:'Unassigned');
  if(groupBy==='leadStatus')return one(row.status,row.status.charAt(0)+row.status.slice(1).toLowerCase());
  if(groupBy==='routing')return one(row.routing,row.routing==='BIXOLON_SALES'?'BIXOLON Sales':row.routing==='REFERRED_TO_PARTNER'?'Referred to Partner':row.routing==='MARKETING_FOLLOW_UP'?'Marketing Follow-Up':'Unreviewed');
  if(groupBy==='referralPartner')return one(String(row.routedPartnerAccountId??'none'),row.routedPartnerAccount?.name??'No referral partner');
  if(groupBy==='competitor')return one(String(row.competitorId??'none'),row.competitor?.name??'No resolved competitor');
  if(groupBy==='opportunityStage')return one(String(row.convertedOpportunity?.stageId??'none'),row.convertedOpportunity?.stage.name??'No Opportunity');
  if(groupBy==='forecastCategory')return one(row.convertedOpportunity?.forecastCategory??'none',row.convertedOpportunity?.forecastCategory.replaceAll('_',' ')??'No Opportunity');
  if(groupBy==='currency')return one(row.convertedOpportunity?.currencyCode??'none',row.convertedOpportunity?.currencyCode??'No Opportunity currency');
  return null;
}
...
export async function executeTradeShowReport(client:PrismaClient,actor:Actor,rawConfig:unknown,now=new Date()):Promise<TradeShowReportResult>{
  ...
  for(const filter of config.filters){const value=filter.value;
    ...
    else if(filter.field==='showDate')clauses.push({tradeShow:{startDate:tradeShowDateBounds(filter,now)}});
    else if(filter.field==='ownerId')clauses.push({assignedSalesRepUserId:value as number});
    else if(filter.field==='leadStatus')clauses.push({status:value as TradeShowLeadStatus});
    else if(filter.field==='routing')clauses.push({routing:value as TradeShowLeadRouting});
    else if(filter.field==='referralPartnerId')clauses.push({routedPartnerAccountId:value as number});
    else if(filter.field==='converted')clauses.push({convertedOpportunityId:value?{not:null}:null});
    else if(filter.field==='accountLinked')clauses.push({accountId:value?{not:null}:null});
    else if(filter.field==='contactLinked')clauses.push({contactId:value?{not:null}:null});
    else if(filter.field==='competitorId')clauses.push({competitorId:value as number});
    else if(filter.field==='productInterest')clauses.push({productInterest:{contains:String(value),mode:'insensitive'}});
    else if(filter.field==='search')clauses.push({OR:[{firstName:{contains:String(value),mode:'insensitive'}},{lastName:{contains:String(value),mode:'insensitive'}},{sourceCompany:{contains:String(value),mode:'insensitive'}},{email:{contains:String(value),mode:'insensitive'}}]});
    else if(filter.field==='stageId')clauses.push({convertedOpportunity:{stageId:value as number}});
    else if(filter.field==='forecastCategory')clauses.push({convertedOpportunity:{forecastCategory:value as ForecastCategory}});
    else if(filter.field==='currency')clauses.push({convertedOpportunity:{currencyCode:value as string}});
    else if(filter.field==='followUpStatus'){
      if(value==='OVERDUE')clauses.push({routing:'BIXOLON_SALES',followUpAt:{lt:now},status:{notIn:['CONVERTED','DISQUALIFIED']}});
      else if(value==='SCHEDULED')clauses.push({routing:'BIXOLON_SALES',followUpAt:{not:null},status:{notIn:['CONVERTED','DISQUALIFIED']}});
      else if(value==='MISSING')clauses.push({routing:'BIXOLON_SALES',followUpAt:null,status:{notIn:['CONVERTED','DISQUALIFIED']}});
      else if(value==='NEW_NOT_CONTACTED')clauses.push({routing:'BIXOLON_SALES',status:'NEW',lastContactedAt:null});
  ...
  for(const row of rows){const group=tradeShowGroup(row,config.groupBy);if(group){const found=groups.get(group.key)??{...group,rows:[]};found.rows.push(row);groups.set(group.key,found);}}
  const detail=rows.map(row=>{const opportunity=row.convertedOpportunity,value=opportunity&&opportunity.archivedAt===null?opportunityTotal(opportunity.products):null;return {id:row.id,tradeShowId:row.tradeShowId,tradeShow:row.tradeShow.name,lead:`${row.firstName} ${row.lastName}`,company:row.sourceCompany??'—',assignedRep:row.assignedSalesRep?`${row.assignedSalesRep.firstName} ${row.assignedSalesRep.lastName}`:'Unassigned',leadStatus:row.status.charAt(0)+row.status.slice(1).toLowerCase(),routing:row.routing==='BIXOLON_SALES'?'BIXOLON Sales':row.routing==='REFERRED_TO_PARTNER'?'Referred to Partner':row.routing==='MARKETING_FOLLOW_UP'?'Marketing Follow-Up':'Unreviewed',referralPartner:row.routedPartnerAccount?{id:row.routedPartnerAccount.id,name:row.routedPartnerAccount.name}:null,account:row.account?{id:row.account.id,name:row.account.name}:null,contact:row.contact?{id:row.contact.id,name:`${row.contact.firstName} ${row.contact.lastName}`}:null,capturedDate:row.capturedAt?.toISOString().slice(0,10)??null,followUp:row.followUpAt?.toISOString().slice(0,10)??null,competitor:row.competitor?.name??'—',competitorSourceText:row.competitorSourceText??'—',productInterest:row.productInterest??'—',opportunity:opportunity?{id:opportunity.id,name:opportunity.name}:null,opportunityOwner:opportunity?.owner?`${opportunity.owner.firstName} ${opportunity.owner.lastName}`:'—',opportunityStage:opportunity?.stage.name??'—',forecastCategory:opportunity?.forecastCategory.replaceAll('_',' ')??'—',opportunityValue:value?.toFixed(2)??null,weightedValue:value&&opportunity?weightedValue(value,opportunity.probability??opportunity.stage.probability).toFixed(2):null,conversionDate:row.convertedAt?.toISOString().slice(0,10)??null,currency:opportunity?.currencyCode??null,canOpenOpportunity:!!opportunity&&can(actor,'sales.read')&&(actor.role!=='SALES'||opportunity.ownerId===actor.id),groupKeys:[tradeShowGroup(row,config.groupBy)?.key].filter((item):item is string=>!!item)};});
  return {reportType:'TRADE_SHOW',summary:tradeShowMetrics(rows),groups:[...groups.values()].sort((a,b)=>a.label.localeCompare(b.label)).map(group=>({key:group.key,label:group.label,metrics:tradeShowMetrics(group.rows),leadIds:group.rows.map(row=>row.id)})),rows:detail,currencies:[...new Set(distinctTradeShowOpportunities(rows).map(item=>item.currencyCode))].sort(),filterCount:config.filters.length,semanticNote:tradeShowDefinition.semanticNote};
}
...
export async function executeReport(client: PrismaClient, actor: Actor, reportType: unknown, rawConfig: unknown, now = new Date()) {
  ...
  if (reportType === 'TRADE_SHOW') return executeTradeShowReport(client,actor,rawConfig,now);
