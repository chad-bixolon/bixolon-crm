...
function productGroupValues(row:ProductLineDbRow, groupBy:string|null):{key:string;label:string}[] {
  ...
  if(groupBy==='odmCustomer')return row.sku?.catalogSource==='ODM'?(row.sku.odmCustomers.length ? row.sku.odmCustomers.map(link=>({key:String(link.accountId),label:link.account.name})) : [{key:'none',label:'Unresolved ODM Customer'}]):[{key:'not-odm',label:'Not ODM'}];
