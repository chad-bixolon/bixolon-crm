...
function productGroupValues(row:ProductLineDbRow, groupBy:string|null):{key:string;label:string}[] {
  ...
  if(groupBy==='odmCustomer')return row.sku?.catalogSource==='ODM'?(row.sku.odmCustomers.length ? row.sku.odmCustomers.map(link=>({key:String(link.accountId),label:link.account.name})) : row.sku.odmSubtype==='CUSTOMER_SPECIFIC' ? [{key:'none',label:'Unresolved ODM Customer'}] : row.sku.odmSubtype ? [{key:'not-customer-specific',label:'No ODM Customer Required'}] : [{key:'subtype-unset',label:'ODM Subtype Not Set'}]):[{key:'not-odm',label:'Not ODM'}];
