...
const priceExceptionUsageDefinition: ReportTypeDefinition = {
  ...
  filters:{ownerId:{label:'Sales Rep',operators:['eq']},accountId:{label:'Account',operators:['eq']},opportunityId:{label:'Opportunity',operators:['eq']},stageId:{label:'Opportunity Stage',operators:['eq']},forecastCategory:{label:'Forecast Category',operators:['eq']},status:{label:'Status',operators:['eq']},closeDate:{label:'Expected Close Date',operators:['preset','between']},productId:{label:'Product',operators:['eq']},skuId:{label:'SKU',operators:['eq']},productCategoryId:{label:'Product Category',operators:['eq']},priceExceptionId:{label:'Price Exception',operators:['eq']},peSalespersonId:{label:'PE Salesperson',operators:['eq']},moqStatus:{label:'MOQ Status',operators:['eq']},overrideStatus:{label:'Override Status',operators:['eq']},currency:{label:'Currency',operators:['eq']}},
...
function validFilterValue(filter: ReportFilter) {
  if (['ownerId','stageId','accountId','accountOwnerId','productCategoryId','productId','skuId','projectId','minDays','opportunityId','priceExceptionId','peSalespersonId'].includes(filter.field)) return positiveInteger(filter.value);
...
export async function executeChannelPartnerReport(client:PrismaClient,actor:Actor,rawConfig:unknown,now=new Date()):Promise<ChannelReportResult>{
  ...
  const opportunities:PipelineDbRow[]=await client.opportunity.findMany({where:pipelineWhere(opportunityConfig,actor,now),include:{stage:true,owner:true,participants:{include:{account:{include:{owner:true,industryCategory:true,territoryCategory:true}},roles:true}},products:{where:{archivedAt:null},include:{product:{include:{category:true}},sku:true}},projects:{include:{project:{include:{owner:true}}}}}});
