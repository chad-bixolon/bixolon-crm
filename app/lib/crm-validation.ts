...
export const partyLabels: Record<OpportunityPartyRole, string> = {
  END_USER: defaultLabels.END_USER, VAR_RESELLER: defaultLabels.VAR, DISTRIBUTOR: defaultLabels.DISTRIBUTOR, ISV_PARTNER: defaultLabels.ISV, OEM: defaultLabels.OEM, OTHER: "Other", MEDIA_PARTNER: defaultLabels.MEDIA_PARTNER, SERVICE_PARTNER: defaultLabels.PARTNER,
};
export function opportunityPartyLabels(labels: LabelMap): Record<OpportunityPartyRole, string> { return { END_USER: labels.END_USER, VAR_RESELLER: labels.VAR, DISTRIBUTOR: labels.DISTRIBUTOR, ISV_PARTNER: labels.ISV, OEM: labels.OEM, OTHER: "Other", MEDIA_PARTNER: labels.MEDIA_PARTNER, SERVICE_PARTNER: "Service Partner" }; }
...
export function friendlyError(error: unknown, fallback: string) {
  if (error instanceof Error && /not found|Reactivate|already|linked|active account|active product|available|primary|MOQ|ODM|Catalog Source|Base SKU|subtype|Part number|Description|reserved/i.test(error.message)) return error.message;
  return fallback;
}
