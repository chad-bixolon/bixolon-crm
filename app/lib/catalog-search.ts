...
export async function searchCatalog(client: PrismaClient, query: string, categoryId: number | null, currencyCode: string) {
  ...
  if (query) where.OR = [
    ...
    { odmCustomers: { some: { archivedAt: null, account: { name: { contains: query, mode: "insensitive" } } } } },
  ];
