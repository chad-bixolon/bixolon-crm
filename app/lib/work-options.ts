...
export async function workOptions(current?: { accountId?: number | null; opportunityId?: number | null; projectId?: number | null; userId?: number | null; contactIds?: number[] }) {
  const [accounts, opportunities, users, activityTypes, projects, contacts] = await Promise.all([
    ...
    prisma.contact.findMany({ where: { OR: [operationalContactWhere, ...(current?.contactIds?.length ? [{ id: { in: current.contactIds } }] : [])] }, select: { id: true, accountId: true, firstName: true, lastName: true, active: true }, orderBy: [{ lastName: 'asc' }, { firstName: 'asc' }] }),
  ]);
  return { accounts, opportunities: opportunities.map(o => ({ id: o.id, name: o.name, accountIds: o.participants.map(p => p.accountId), projectIds: o.projects.map(p => p.projectId) })), projects: projects.map(p => ({ id: p.id, name: p.name, accountIds: [...(p.primaryAccountId ? [p.primaryAccountId] : []), ...p.participants.map(a => a.accountId)], opportunityIds: p.opportunities.map(o => o.opportunityId) })), contacts: contacts.map(c => ({ id: c.id, accountId: c.accountId, active: c.active, name: `${c.firstName} ${c.lastName}` })), users: users.map(u => ({ id: u.id, name: `${u.firstName} ${u.lastName}` })), activityTypes };
}
