...
export async function workOptions() {
  const [accounts, opportunities, users, activityTypes, projects] = await Promise.all([
  ...
  return { accounts, opportunities, projects, users: users.map(u => ({ id: u.id, name: `${u.firstName} ${u.lastName}` })), activityTypes };
}
