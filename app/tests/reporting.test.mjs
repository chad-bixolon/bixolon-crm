...
test('report discovery follows runnable type and built-in authorization',()=>{
 const salesBuiltIns=['MY_OPEN_PIPELINE','PIPELINE_THIS_QUARTER','PIPELINE_BY_SALES_REP','ACCOUNT_ENGAGEMENT','PRODUCT_THIS_QUARTER','PIPELINE_BY_PRODUCT'];
 for(const role of ['ADMIN','SALES_MANAGER','SALES']){
  assert.deepEqual(reporting.getVisibleBuiltInReports(actor(role)),salesBuiltIns);
  assert.deepEqual(reporting.getVisibleReportTypes(actor(role)),['PIPELINE','ACCOUNT_ACTIVITY','PRODUCT_PERFORMANCE']);
  assert.deepEqual(reporting.getCreatableReportTypes(actor(role)),['PIPELINE','ACCOUNT_ACTIVITY','PRODUCT_PERFORMANCE']);
 ...
 assert.deepEqual(reporting.getVisibleReportTypes(actor('READ_ONLY')),['PIPELINE','PRODUCT_PERFORMANCE']);
 ...
 for(const reportType of reporting.reportTypes.filter(type=>!['PIPELINE','ACCOUNT_ACTIVITY','PRODUCT_PERFORMANCE'].includes(type)))assert.equal(reporting.canRunReportType(actor('ADMIN'),reportType),false);
});
...
test('Sales scope is always applied even when a shared report requests another owner',async()=>{
 ...
 assert.deepEqual(reporting.savedReportWhere(actor('SALES',7)),{archivedAt:null,reportType:{in:['PIPELINE','ACCOUNT_ACTIVITY','PRODUCT_PERFORMANCE']},OR:[{ownerId:7},{visibility:'SHARED'}]});
});
