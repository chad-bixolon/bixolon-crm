...
export function PriceExceptionResolutionForm({id,parties,salesReps,assignedSalesRepUserId}:{id:number;parties:Party[];salesReps:SalesRep[];assignedSalesRepUserId:number|null}){
  ...
  return <section className="panel mb-5 border-orange-200 p-6" aria-labelledby="resolve-references-heading">
    <div className="mb-6 border-b border-slate-200 pb-6"><h2 className="text-lg font-semibold">Assign BIXOLON Sales Rep</h2><p className="mt-1 text-sm text-slate-600">This CRM ownership link is separate from imported source and distributor salesperson values.</p><form action={repAction} className="mt-4 flex flex-wrap items-end gap-3"><label className="min-w-72 flex-1 text-xs font-semibold">SalesHub user<select className="field mt-1" name="assignedSalesRepUserId" defaultValue={repState.value??(assignedSalesRepUserId?String(assignedSalesRepUserId):'')}><option value="">Clear assignment</option>{salesReps.map(rep=><option key={rep.id} value={rep.id}>{rep.firstName} {rep.lastName}{!rep.active||rep.archivedAt?' (inactive / historical)':''}</option>)}</select>{repState.error&&<span className="mt-1 block text-red-700">{repState.error}</span>}</label><button className="btn-primary" disabled={repPending} type="submit">{repPending?'Saving…':'Save Sales Rep'}</button>{repState.saved&&<span className="text-sm text-green-700">Saved.</span>}</form></div>
