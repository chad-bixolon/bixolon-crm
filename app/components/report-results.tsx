...
import type { PipelineReportResult, AccountActivityReportResult, ProductPerformanceResult, ReportConfiguration } from '@/lib/reporting';
...
export function ReportResults({ result, config, groupKey }: { result: PipelineReportResult|AccountActivityReportResult|ProductPerformanceResult; config: ReportConfiguration; groupKey?: string }) {
  return result.reportType==='ACCOUNT_ACTIVITY'?<AccountResults result={result} config={config} groupKey={groupKey}/>:result.reportType==='PRODUCT_PERFORMANCE'?<ProductResults result={result} config={config} groupKey={groupKey}/>:<PipelineResults result={result} config={config} groupKey={groupKey}/>;
}
