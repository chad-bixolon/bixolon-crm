...
export function AccountForm({ id, initial, industries, territories, owners, labels, leadContext }: { id?: number; initial?: Initial; industries: Option[]; territories: Option[]; owners: Owner[]; labels?: LabelMap; leadContext?: {tradeShowId:number;leadId:number} }) {
  ...
  return <form action={action} onSubmit={guard} className="panel max-w-4xl p-6 lg:p-8" aria-label={id ? "Edit account" : "Create account"}>
    ...
    <div className="grid gap-6 sm:grid-cols-2">
      ...
      <div><label className="label" htmlFor="territory">Territory</label><select className="field" id="territory" name="territory" defaultValue={initial?.territory ?? ""}><option value="">None selected</option>{territories.map((o) => <option key={o.code} value={o.code}>{o.name}{o.active === false ? " (inactive)" : ""}</option>)}{initial?.territory && !territories.some((o) => o.code === initial.territory) && <option value={initial.territory}>{initial.territory} (inactive)</option>}</select><p className="mt-1 text-xs text-slate-500">Sales coverage for this Account.</p>{error("territory")}</div>
      ...
      <label className="sm:col-span-2"><span className="flex items-center gap-3 text-sm font-medium text-slate-700"><input type="checkbox" name="strategicAccount" defaultChecked={initial?.strategicAccount} className="accent-orange-700"/>{labels?.STRATEGIC_ACCOUNT ?? "Strategic Account"}</span><span className="mt-1 block text-xs font-normal text-slate-500">Identifies an Account requiring strategic or national-level attention.</span></label>
