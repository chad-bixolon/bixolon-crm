...
const manualPricing = (): PricingChange => ({ priceSource: "MANUAL", catalogPriceTier: null, priceExceptionLineId: null, priceExceptionCode: null, priceExceptionUnitPrice: null, priceExceptionCurrencyCode: null, priceExceptionSourceQty: null, priceExceptionAccountIds: [], odmCustomerPriceId: null, odmCustomerAccountId: null, odmCustomerBasePrice: null, odmCustomerTariffPercent: null, odmCustomerTariffAmount: null, odmCustomerFinalUnitPrice: null });
...
export function ProductPicker(props: Props) {
  ...
  const prices = item && !(item.catalogSource === 'ODM' && item.odmSubtype === 'CUSTOMER_SPECIFIC') ? pricesForCurrency(item, currencyCode) : [];
  const odmMatches = matchingOdmCustomers(item, accountIds, currencyCode);
  const selectOdm = (customer: typeof odmMatches[number]) => { if (!customer.price) return; onChange(productId, skuId, customer.price.finalUnitPrice, { ...manualPricing(), priceSource: 'ODM_CUSTOMER', odmCustomerPriceId: customer.price.id, odmCustomerAccountId: customer.accountId, odmCustomerBasePrice: customer.price.customerPrice, odmCustomerTariffPercent: customer.price.tariffPercent, odmCustomerTariffAmount: customer.price.tariffAmount, odmCustomerFinalUnitPrice: customer.price.finalUnitPrice }); setShowPe(false); };
  ...
  const choose = (selected: CatalogItem) => {
    ...
    const matches = matchingOdmCustomers(selected, accountIds, currencyCode);
    if (selected.catalogSource === 'ODM' && selected.odmSubtype === 'CUSTOMER_SPECIFIC') {
      const matched = matches.length === 1 ? matches[0] : null;
      onChange(choice.productId, choice.skuId, matched?.price?.finalUnitPrice ?? '0.00', matched?.price ? { ...manualPricing(), priceSource: 'ODM_CUSTOMER', odmCustomerPriceId: matched.price.id, odmCustomerAccountId: matched.accountId, odmCustomerBasePrice: matched.price.customerPrice, odmCustomerTariffPercent: matched.price.tariffPercent, odmCustomerTariffAmount: matched.price.tariffAmount, odmCustomerFinalUnitPrice: matched.price.finalUnitPrice } : manualPricing());
    } else onChange(choice.productId, choice.skuId, choice.price, choice.tier ? { ...manualPricing(), priceSource: "CATALOG", catalogPriceTier: choice.tier } : manualPricing());
  };
  ...
  const renderPeOption = (option: Candidate, state: "ELIGIBLE" | "INELIGIBLE" | "UNKNOWN") => {
    ...
    }}><span className="font-semibold">PE {option.peCode ?? "Unnumbered"} · {formatCurrency(Number(option.unitPrice), option.currencyCode)}</span><span className="block text-slate-600">{option.moq ? `MOQ ${option.moq}` : `MOQ unresolved${option.moqRaw ? `: ${option.moqRaw}` : ""}`} · Expires: {option.expirationDate ?? "—"}</span>{partyText && <span className="block text-slate-600">{partyText}</span>}{!option.matchedRoles.length && <span className="block text-amber-700">No linked Price Exception Account matches this Opportunity.</span>}{state === "INELIGIBLE" && <span className="block text-amber-700">Requires quantity of at least {option.moq}.</span>}{state === "UNKNOWN" && <span className="block text-amber-700">A numeric MOQ is required before this price can be applied.</span>}{option.comments && <span className="block truncate text-slate-500">{option.comments}</span>}</button>;
  };
  const selectedMoqState = pricing.priceSource === "PRICE_EXCEPTION" ? moqState(quantity, pricing.priceExceptionSourceQty) : null;
  return <div className="relative min-w-0">
    <input type="hidden" name="productId" value={productId || ""}/><input type="hidden" name="skuId" value={skuId || ""}/><input type="hidden" name="priceSource" value={pricing.priceSource}/><input type="hidden" name="catalogPriceTier" value={pricing.catalogPriceTier ?? ""}/><input type="hidden" name="priceExceptionLineId" value={pricing.priceExceptionLineId ?? ""}/><input type="hidden" name="odmCustomerPriceId" value={pricing.odmCustomerPriceId ?? ""}/><input type="hidden" name="odmCustomerAccountId" value={pricing.odmCustomerAccountId ?? ""}/>
