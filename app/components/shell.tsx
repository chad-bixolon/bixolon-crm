...
const nav = ["Dashboard", "Accounts", "Contacts", "Trade Shows", "Marketing Audiences", "Projects", "Opportunities", "Pipeline", "Reports", "Tasks", "Products", "Price Exceptions", "Administration", "Integrations"];
const hrefFor = (item: string) => item === "Dashboard" ? "/" : item === "Price Exceptions" ? "/price-exceptions" : item === "Trade Shows" ? "/trade-shows" : item === "Marketing Audiences" ? "/marketing/audiences" : `/${item.toLowerCase()}`;
...
type ShellUser = { name: string; role: UserRole; canManageUsers: boolean; canViewReports: boolean; canViewMarketing:boolean };
  ...
  return <div className="min-h-screen lg:flex">
    <aside className="border-b border-slate-200 bg-white lg:sticky lg:top-0 lg:h-screen lg:w-60 lg:shrink-0 lg:border-b-0 lg:border-r">
      ...
      <nav aria-label="Primary navigation" className="flex gap-1 overflow-x-auto px-3 pb-3 lg:block lg:space-y-1 lg:py-5">
        {nav.filter((item) => (item !== "Administration" || user?.canManageUsers) && (item !== "Reports" || user?.canViewReports) && (item !== "Marketing Audiences" || user?.canViewMarketing) && (item !== "Trade Shows" || !!user)).map((item) => { const href = hrefFor(item); const active = href === "/" ? pathname === "/" : pathname.startsWith(href);
