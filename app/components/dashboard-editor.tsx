...
import type { DashboardLayoutConfiguration, DashboardLayoutItem, DashboardWidgetKey, DashboardWidgetSize, SavedReportWidgetStyle } from '@/lib/dashboard';

type AvailableWidget={key:DashboardWidgetKey;title:string;description:string;sizes:readonly DashboardWidgetSize[];defaultSize:DashboardWidgetSize};
  ...
  return <form action={formAction} className="space-y-5"><input type="hidden" name="configuration" value={JSON.stringify({version:1,items})}/>
    <section className="panel overflow-hidden"><div className="border-b px-4 py-3"><h2 className="font-semibold">Visible widgets</h2><p className="text-sm text-slate-600">Use the arrow controls to set the reading order.</p></div>
      {items.length?<ol className="divide-y">{items.map((item,index)=>{const report=item.kind==='SAVED_REPORT'?reports.find(value=>value.id===item.reportId):null;const widget=item.kind==='BUILTIN'?definition(item.key):null;const heading=widget?.title??(item.kind==='SAVED_REPORT'?item.title:undefined)??report?.name??'Report unavailable';return <li className="grid gap-3 p-4 lg:grid-cols-[minmax(0,1fr)_auto_auto_auto] lg:items-center" key={item.kind==='BUILTIN'?item.key:`report-${item.reportId}`}><div className="min-w-0"><p className="font-medium">{heading}</p><p className="text-xs text-slate-500">{item.kind==='SAVED_REPORT'?`${report?.reportType??'Saved Report'} · Saved Report`:widget?.description}</p></div><div className="flex gap-1"><button className="btn-secondary px-3" type="button" disabled={index===0} onClick={()=>move(index,-1)} aria-label={`Move ${heading} up`}>↑</button><button className="btn-secondary px-3" type="button" disabled={index===items.length-1} onClick={()=>move(index,1)} aria-label={`Move ${heading} down`}>↓</button></div>
        ...
        <button className="text-sm font-semibold text-red-700 underline" type="button" onClick={()=>remove(index)}>Remove</button></li>;})}</ol>:<p className="p-4 text-sm text-slate-500">No widgets selected.</p>}
    </section>
    <section className="panel p-4"><h2 className="font-semibold">Available widgets</h2><div className="mt-3 grid gap-2 sm:grid-cols-2">{widgets.filter(widget=>!visibleKeys.has(widget.key)).map(widget=><button className="rounded-md border p-3 text-left hover:border-orange-300 hover:bg-orange-50" type="button" key={widget.key} onClick={()=>addBuiltin(widget.key)}><span className="font-medium">+ {widget.title}</span><span className="mt-1 block text-xs text-slate-600">{widget.description}</span></button>)}</div>{widgets.every(widget=>visibleKeys.has(widget.key))&&<p className="mt-2 text-sm text-slate-500">All available widgets are shown.</p>}</section>
