...
export type SkuInitial = { id: number; partNumber: string; description: string | null; active: boolean; catalogSource: string | null; odmSubtype: string | null; odmDescription: string | null; odmCustomers: { account: { id: number; name: string } }[]; baseSku: { id: number; partNumber: string; product: { name: string } } | null };
export function ProductSkuForm({ productId, initial }: { productId: number; initial?: SkuInitial }) {
  ...
  return <form action={action} className="space-y-3 rounded border border-slate-200 p-4 text-sm">
    ...
    <div className="grid gap-3 sm:grid-cols-2"><label className="label">Part number<input className="field" name="partNumber" required maxLength={100} defaultValue={initial?.partNumber}/></label><CatalogSourceField source={source} onChange={setSource}/></div>
    ...
    <label className="label">Status<select className="field" name="active" defaultValue={initial?.active === false ? 'false' : 'true'}><option value="true">Active</option><option value="false">Inactive</option></select></label>
    {source === 'ODM' && <OdmDetailsFields initial={initial}/>}
  </form>;
}
