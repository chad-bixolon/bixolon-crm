...
export type SkuInitial = { id: number; partNumber: string; description: string | null; catalogSource: string | null; odmSubtype: string | null; odmDescription: string | null; odmCustomers: { account: { id: number; name: string } }[]; baseSku: { id: number; partNumber: string; product: { name: string } } | null };
export function ProductSkuForm({ productId, initial }: { productId: number; initial?: SkuInitial }) {
  ...
  return <form action={action} className="space-y-3 rounded border border-slate-200 p-4 text-sm">
    ...
    {source === 'ODM' && <div className="grid gap-3 sm:grid-cols-2"><label className="label">ODM subtype<select className="field" name="odmSubtype" defaultValue={initial?.odmSubtype ?? ''}><option value="">{initial ? 'Historical: unclassified' : 'Choose subtype'}</option>{Object.entries(odmSubtypeLabels).filter(([value])=>value!=='LEGACY_SPECIAL_SKU'||initial?.odmSubtype==='LEGACY_SPECIAL_SKU').map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><AccountChoices initial={initial?.odmCustomers}/><SearchChoice kind="baseSku" name="baseSkuId" label="Base SKU" initial={initial?.baseSku ? { id: initial.baseSku.id, label: `${initial.baseSku.partNumber} · ${initial.baseSku.product.name}` } : null}/><label className="label sm:col-span-2">ODM description / customization notes<input className="field" name="odmDescription" maxLength={2000} defaultValue={initial?.odmDescription ?? ''}/></label></div>}
  </form>;
}
