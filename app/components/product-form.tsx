...
export function ProductForm({ id, initial, categories = [] }: { id?: number; initial?: { sku: string; name: string; active: boolean; categoryId: number | null }; categories?: { id: number; name: string }[] }) {
  ...
  return <form key={state.values ? JSON.stringify(state.values) : 'initial'} action={action} onSubmit={guard} className="panel max-w-2xl space-y-4 p-6" aria-label={id ? "Edit product" : "Create product"}>
