...
type Initial = { name: string; description: string | null; ownerId: number | null; stageId: number; expectedCloseDate: Date | null; probability: number | null; forecastCategory: ForecastCategory | null; currencyCode: string; participants: { accountId: number; roles: OpportunityPartyRole[] }[]; lines: { id: number; productId: number; quantity: number; price: string }[] };
type Props = { id?: number; initial?: Initial; accounts: Option[]; owners: { id: number; firstName: string; lastName: string }[]; stages: { id: number; name: string; probability: number }[]; currencies: { code: string; name: string }[]; products: { id: number; name: string; sku: string }[] };
function initialDraft(initial?: Initial): OpportunityDraft {
  return { name: initial?.name ?? "", description: initial?.description ?? "", ownerId: initial?.ownerId?.toString() ?? "", stageId: initial?.stageId?.toString() ?? "", expectedCloseDate: initial?.expectedCloseDate?.toISOString().slice(0, 10) ?? "", probability: initial?.probability?.toString() ?? "", forecastCategory: initial?.forecastCategory ?? "", currencyCode: initial?.currencyCode ?? "USD", participants: initial?.participants ?? [], lines: initial?.lines.map((line) => ({ ...line, quantity: String(line.quantity) })) ?? [] };
}
export function OpportunityForm({ id, initial, accounts, owners, stages, currencies, products }: Props) {
