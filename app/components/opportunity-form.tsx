...
export function OpportunityForm({ id, initial, accounts, projects, productCategories, owners, stages, currencies, productCount, labels }: Props) {
  ...
  const [lineKeys, setLineKeys] = useState(() => initial?.lines.map((_, index) => index) ?? []);
  const nextLineKey = useRef(initial?.lines.length ?? 0);
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      if (!active) return;
      const restored = restoreDraft(localStorage, draftKey, original.current);
      setDraft(restored);
      setLineKeys(restored.lines.map((_, index) => index));
      nextLineKey.current = restored.lines.length;
  ...
  return <form action={action} onSubmit={guard} className="panel max-w-5xl space-y-7 p-6" aria-label={id ? "Edit opportunity" : "Create opportunity"}>
    ...
    <section><div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-semibold">Opportunity products</h2>{productCount > 0 && <button type="button" className="btn-secondary" onClick={() => { const key = nextLineKey.current++; setLineKeys([...lineKeys, key]); update("lines", [...draft.lines, { id: 0, productId: 0, skuId: 0, quantity: "1", price: "0.00" }]); }}>Add product</button>}</div>{error("lines")}
      ...
      <div className="space-y-3">{draft.lines.map((line, index) => <div key={line.id || `new-${lineKeys[index]}`} className="grid items-start gap-3 rounded border border-slate-200 p-3 sm:grid-cols-[minmax(0,2fr)_minmax(0,3fr)]"><input type="hidden" name="lineId" value={line.id || ""}/><ProductPicker categories={productCategories} index={index} productId={line.productId} skuId={line.skuId ?? 0} currencyCode={draft.currencyCode} price={line.price} onChange={(productId, skuId, price) => update("lines", draft.lines.map((item, i) => i === index ? { ...item, productId, skuId, price: price ?? item.price } : item))}/><div className="grid grid-cols-[minmax(0,1fr)_minmax(0,1.3fr)_minmax(0,1.4fr)] items-end gap-2"><label className="text-xs text-slate-600">Qty<input className="field mt-1" aria-label={`Quantity ${index + 1}`} name="quantity" type="number" min="1" step="1" value={line.quantity} onChange={(e) => update("lines", draft.lines.map((item, i) => i === index ? { ...item, quantity: e.target.value } : item))}/></label><label className="text-xs text-slate-600">Unit price<input className="field mt-1" aria-label={`Estimated unit price ${index + 1}`} name="price" type="number" min="0" step="0.01" value={line.price} onChange={(e) => update("lines", draft.lines.map((item, i) => i === index ? { ...item, price: e.target.value } : item))}/></label><div className="self-center text-xs text-slate-600">Total<span className="mt-1 block break-words text-sm font-semibold tabular-nums text-slate-900">{formatCurrency(Number(line.quantity) * Number(line.price || 0), draft.currencyCode)}</span></div><button type="button" className="col-span-3 justify-self-end text-xs text-slate-600 underline" onClick={() => { setLineKeys(lineKeys.filter((_, i) => i !== index)); update("lines", draft.lines.filter((_, i) => i !== index)); }}>Remove</button></div></div>)}</div>{draft.lines.length > 0 && <p className="mt-2 text-xs text-slate-500">Removing a saved line archives it. Total uses unarchived lines only.</p>}
