...
type Initial = { name: string; description: string | null; ownerId: number | null; projectId: number | null; stageId: number; expectedCloseDate: Date | null; probability: number | null; forecastCategory: ForecastCategory | null; currencyCode: string; participants: { accountId: number; roles: OpportunityPartyRole[] }[]; lines: { id: number; productId: number; quantity: number; price: string }[] };
type Props = { id?: number; initial?: Initial; accounts: Option[]; projects: Option[]; owners: { id: number; firstName: string; lastName: string }[]; stages: { id: number; name: string; probability: number }[]; currencies: { code: string; name: string }[]; products: { id: number; name: string; sku: string }[] };
function initialDraft(initial?: Initial): OpportunityDraft {
  return { name: initial?.name ?? "", description: initial?.description ?? "", ownerId: initial?.ownerId?.toString() ?? "", projectId: initial?.projectId?.toString() ?? "", stageId: initial?.stageId?.toString() ?? "", expectedCloseDate: initial?.expectedCloseDate?.toISOString().slice(0, 10) ?? "", probability: initial?.probability?.toString() ?? "", forecastCategory: initial?.forecastCategory ?? "", currencyCode: initial?.currencyCode ?? "USD", participants: initial?.participants ?? [], lines: initial?.lines.map((line) => ({ ...line, quantity: String(line.quantity) })) ?? [] };
}
export function OpportunityForm({ id, initial, accounts, projects, owners, stages, currencies, products }: Props) {
  ...
  return <form action={action} onSubmit={guard} className="panel max-w-5xl space-y-7 p-6" aria-label={id ? "Edit opportunity" : "Create opportunity"}>
    ...
    <div className="grid gap-5 sm:grid-cols-2">
      ...
      <div><label className="label" htmlFor="projectId">Project</label><select className="field" id="projectId" name="projectId" value={draft.projectId ?? ''} onChange={(e) => update('projectId', e.target.value)}><option value="">No Project</option>{projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}{initial?.projectId && !projects.some(p => p.id === initial.projectId) && <option value={initial.projectId}>Current Project (archived)</option>}</select>{error('projectId')}</div>
    ...
    <section><h2 className="mb-3 text-lg font-semibold">Participating accounts</h2><p className="mb-4 text-sm text-slate-600">Add the companies involved in this opportunity and select the role each company plays in this specific deal. These roles do not change the Account&apos;s normal business roles.</p>{error("participants")}
      ...
      <div className="space-y-4">{draft.participants.map((p) => <div key={p.accountId} className="rounded border border-slate-200 p-4"><div className="flex items-center justify-between gap-2"><strong>{accountOptions.find((a) => a.id === p.accountId)?.name ?? `Account #${p.accountId}`}</strong><button type="button" className="btn-secondary" onClick={() => setDraft((old) => removeParticipant(old, p.accountId))}>Remove account</button></div><input type="hidden" name="accountId" value={p.accountId}/><p className="mt-2 text-sm text-slate-600">Opportunity roles: {p.roles.map((role) => partyLabels[role]).join(", ") || "None selected"}</p><div className="mt-3 flex flex-wrap gap-3">{Object.entries(partyLabels).map(([role, label]) => <label key={role} className="flex items-center gap-1 text-sm"><input type="checkbox" checked={p.roles.includes(role as OpportunityPartyRole)} onChange={(e) => setDraft((old) => setParticipantRoles(old, p.accountId, e.target.checked ? [...p.roles, role as OpportunityPartyRole] : p.roles.filter((r) => r !== role)))}/>{label}</label>)}</div><input type="hidden" name="participantRoles" value={p.roles.join(",")}/></div>)}</div>
    </section>
