...
export function ContactForm({ id, initial, accounts, accountId }: { id?: number; initial?: Initial; accounts: { id: number; name: string }[]; accountId?: number }) {
  ...
  return <form action={action} className="panel max-w-4xl p-6" aria-label={id ? "Edit contact" : "Create contact"}>
    {state.message && <p role="alert" className="mb-5 rounded bg-red-50 p-3 text-sm text-red-800">{state.message}</p>}
    <div className="grid gap-5 sm:grid-cols-2">
      <div className="sm:col-span-2"><label className="label" htmlFor="accountId">Account *</label><select className="field" name="accountId" id="accountId" required defaultValue={initial?.accountId ?? accountId ?? ""}><option value="">Choose account</option>{accounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}{initial && !accounts.some((a) => a.id === initial.accountId) && <option value={initial.accountId}>Current account (inactive)</option>}</select>{error("accountId")}</div>
    ...
    </div><div className="mt-7 flex justify-end gap-2"><Link href={id ? `/contacts/${id}` : "/contacts"} className="btn-secondary">Cancel</Link><button className="btn-primary disabled:opacity-60" disabled={pending}>{pending ? "Saving…" : id ? "Save contact" : "Create contact"}</button></div>
  </form>;
}
