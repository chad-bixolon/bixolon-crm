...
Expiration dates accept `M/D/YY`, `MM/DD/YY`, `M/D/YYYY`, `MM/DD/YYYY`, and `YYYY-MM-DD`. Two-digit years mean 20YY. Dates must be real calendar dates within 1900–2100. The canonical date is stored on the PE; the exact source text remains in `rosaRawRows`.

Rows are grouped by normalized PE Number. A group is READY when all required source values and relationships are valid and resolved; harmless Description whitespace is ignored for this comparison. Each source pricing row becomes its own `PriceExceptionLine`, including repeated SKUs with different quantities or prices. Stable source line keys use a tier fingerprint and occurrence number, not a uniqueness constraint on SKU. Missing or ambiguous user or Account matches require review, with the original party names retained. The required SKU must resolve; an unresolved SKU, invalid date, price, quantity, currency, or required source field is an error. Conflicting header fields require review and are listed with source line values. An existing Rosa PE is marked EXISTING / NO CHANGE only when its stored header and full tier multiset match and no relationship needs review; different tiers require review. Apply uses a serializable transaction and repeats the preview before writing. Existing PE lines and Opportunity snapshots are never updated or deleted.

Administrators can resolve unmatched Customer, VAR, and End User Accounts, unmatched Requested By and Reviewed By Users, and unmatched SKU lines in the preview by selecting active CRM records. They can also choose a source line for conflicts in requested/reviewed timestamps, requested/reviewed names, party names, and description. Each selection triggers a fresh server-side plan; Apply repeats that plan inside the transaction and compares its digest with the reviewed preview. All original CSV rows remain in `rosaRawRows`; selected values and IDs are stored separately in `rosaReviewedChoices`. Invalid source dates, pricing, quantities, currency, status, and required source fields remain blocked. Existing PEs are never overwritten.
